...
func FindUsers() []User {
	var users []User
	OldDB.Select("user.*,role.name role_name,role_id").Joins("left join user_role on user.id=user_role.user_id").Joins("left join role on user_role.role_id=role.id").Order("user.id desc").Find(&users)
	return users
}
